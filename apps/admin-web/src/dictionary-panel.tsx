...
type ItemModalState =
  | { type: "create" }
  | { type: "detail"; item: DictionaryItem }
  | { type: "edit"; item: DictionaryItem }
  | null;
...
export function DictionaryPanel({ api, onNavigate }: { api: DictionaryPanelApi; onNavigate?(path: string): void }) {
    ...
    () => [
      {
        ...
        render: (dictionary) => (
          <button
            type="button"
            style={nameLinkStyle}
            onClick={() => onNavigate?.(`/admin/dictionaries/${dictionary.id}/items`)}
          >
            {dictionary.name}
          </button>
        )
    ...
    <section aria-label="字典管理面板" className="ui-admin-page" style={pageStyle}>
      ...
      <section className="ui-admin-card" aria-label="字典数据展示区" style={dataRegionStyle} aria-busy={loading}>
        <form className="ui-admin-filters" style={filterFormStyle} onSubmit={(event) => void handleQuery(event)}>
          <ClearableFilterInput id="dictionary_keyword" label="关键字" placeholder="输入字典编码、名称或备注" value={keyword} onChange={setKeyword} />
          <ClearableFilterSelect id="dictionary_status" label="状态" placeholder="请选择状态" value={status} onChange={setStatus}>
...
export function DictionaryItemPanel({ api, dictionaryId, onNavigate }: { api: DictionaryPanelApi; dictionaryId: number; onNavigate?(path: string): void }) {
  const [loading, setLoading] = useState(true);
  const [errorMessage, setErrorMessage] = useState("");
  ...
  const [total, setTotal] = useState(0);
  const [selectedIDs, setSelectedIDs] = useState<FixedActionListRowId[]>([]);
  const [modal, setModal] = useState<ItemModalState>(null);
  const [itemForm, setItemForm] = useState<DictionaryItemInput>(defaultItemForm);
  const didLoadRef = useRef(false);

  const pageCount = Math.max(1, Math.ceil(total / pageSize));
  const columns = useMemo<Array<FixedActionListColumn<DictionaryItem>>>(
    () => [
      {
        key: "label",
        title: "字段名称",
        render: (item) => (
          <button type="button" style={nameLinkStyle} onClick={() => setModal({ type: "detail", item })}>
            {item.label}
          </button>
        )
      },
  ...
  async function loadInitial() {
    ...
    setErrorMessage("");
    try {
      const [dictionaryResult, itemResult] = await Promise.all([
        api.listDictionaries({ page: 1, page_size: 1000 }),
        api.listDictionaryManageItems(dictionaryId, buildItemQuery("", 1, defaultPageSize))
      ]);
      setDictionary(dictionaryResult.items.find((item) => item.id === dictionaryId) ?? null);
      applyItemResult(itemResult, buildItemQuery("", 1, defaultPageSize));
    } catch (error) {
      setDictionary(null);
      setItems([]);
      setTotal(0);
      setErrorMessage(error instanceof Error ? normalizeErrorMessage(error.message) : "字典项数据加载失败");
    } finally {
      setLoading(false);
    }
  }

  async function loadItems(query: DictionaryItemManageListQuery = buildItemQuery(status, page, pageSize)) {
    setLoading(true);
    setErrorMessage("");
    try {
      const result = await api.listDictionaryManageItems(dictionaryId, query);
      applyItemResult(result, query);
    } catch (error) {
      setItems([]);
      setTotal(0);
      setPage(query.page || 1);
  ...
  function applyItemResult(result: PageResult<DictionaryItem>, query: DictionaryItemManageListQuery) {
  ...
  async function handleQuery(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setSelectedIDs([]);
    await loadItems(buildItemQuery(status, 1, pageSize));
  }

  async function handleReset() {
    setStatus("");
    setSelectedIDs([]);
    await loadItems(buildItemQuery("", 1, defaultPageSize));
  }

  async function handlePageChange(nextPage: number) {
    setSelectedIDs([]);
    await loadItems(buildItemQuery(status, nextPage, pageSize));
  }
  ...
  async function handleItemSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (modal?.type === "edit") {
        await api.updateDictionaryItem(modal.item.id, payload);
      } else {
        await api.createDictionaryItem(dictionaryId, payload);
      }
      closeModal();
      await loadItems();
    } catch (error) {
      setErrorMessage(error instanceof Error ? normalizeErrorMessage(error.message) : "字典项保存失败");
    }
  }
  ...
  function openEditModal(item: DictionaryItem) {
    setItemForm({
      value: item.value,
      label: item.label,
  ...
  function handleExport() {
    downloadCsv(
      ...
      [
        ...
        { key: "label", title: "字段名称" },
        { key: "value", title: "字段值" },
        { key: "sort_no", title: "排序" },
        { key: "status_label", title: "状态" },
        { key: "remark", title: "备注" }
      ],
      items.map((item) => ({
        ...
        dictionary_code: dictionary?.code ?? item.dictionary_code ?? "",
  ...
  return (
    <section aria-label="字典项管理面板" className="ui-admin-page" style={pageStyle}>
      {errorMessage ? (
        <ToastNotice tone="danger" title="字典项数据加载失败" description={errorMessage} onClose={() => setErrorMessage("")} />
      ) : null}

      <section className="ui-admin-card" aria-label="字典项数据展示区" style={dataRegionStyle} aria-busy={loading}>
        <form className="ui-admin-filters" style={filterFormStyle} onSubmit={(event) => void handleQuery(event)}>
          <ClearableFilterSelect id="dictionary_item_status" label="状态" placeholder="请选择状态" value={status} onChange={setStatus}>
            <option value="active">启用</option>
            <option value="disabled">停用</option>
          </ClearableFilterSelect>
          <div className="ui-admin-actions-bar__group" style={queryActionsStyle}>
            ...
            <button type="button" className="ui-button ui-button--ghost" onClick={() => onNavigate?.("/admin/dictionaries")}>
              返回字典
            </button>
          </div>
        </form>
        ...
        <FixedActionList
          ...
          onPageChange={(nextPage) => void handlePageChange(nextPage)}
          minHeight="100%"
          emptyText={loading ? "数据加载中..." : "暂无字典项数据"}
          ariaLabel="字典项列表"
          createLabel="新增"
          deleteLabel="删除"
          exportLabel="导出"
      ...
      {modal ? (
        <div className="ui-admin-modal-backdrop">
          <section className="ui-admin-modal" aria-label="字典项管理弹层">
            {modal.type === "detail" ? (
              <>
                <div className="ui-admin-modal__header">
                  <div>
                    <h3>字典项详情</h3>
                    <p>{modal.item.label}</p>
                  </div>
                  <button type="button" className="ui-button ui-button--ghost" onClick={closeModal}>
                    关闭
                  </button>
                ...
                <div className="ui-admin-modal__body">
                  <dl className="ui-admin-meta-list">
                    <div>
                      <dt>字段名称</dt>
                      <dd>{modal.item.label}</dd>
                    </div>
                    <div>
                      <dt>字段值</dt>
                      <dd>{modal.item.value}</dd>
                    </div>
                    <div>
                      <dt>排序</dt>
                      <dd>{modal.item.sort_no}</dd>
                    </div>
                    <div>
                      <dt>状态</dt>
                      <dd>
                        <span className={statusClassName(modal.item.status)}>{formatStatusLabel(modal.item.status)}</span>
                      </dd>
                    </div>
                    <div>
                      <dt>备注</dt>
              ...
              <form onSubmit={(event) => void handleItemSubmit(event)}>
                <div className="ui-admin-modal__header">
                  <div>
                    <h3>{modal.type === "create" ? "新增字典项" : "编辑字典项"}</h3>
                    <p>{dictionary?.name ?? `字典-${dictionaryId}`}</p>
                  </div>
                  <button type="button" className="ui-button ui-button--ghost" onClick={closeModal}>
                    关闭
                  </button>
...
function DictionaryItemForm({ form, onChange }: { form: DictionaryItemInput; onChange(next: DictionaryItemInput): void }) {
  return (
    <div className="ui-admin-form__grid">
      <div className="ui-admin-form__field">
        <label htmlFor="dictionary_item_label">字段名称</label>
      ...
      <div className="ui-admin-form__field">
        ...
        <input
          ...
          onChange={(event) => onChange({ ...form, sort_no: Number(event.target.value) })}
        />
      </div>
      <div className="ui-admin-form__field">
...
function buildItemQuery(status: string, page: number, pageSize: number): DictionaryItemManageListQuery {
  return {
    status: status || undefined,
    page,
    page_size: pageSize
  } as DictionaryItemManageListQuery;
}
...
function buildItemPayload(form: DictionaryItemInput): DictionaryItemInput | string {
  if (!form.label.trim()) {
    return "请填写字段名称。";
  }
  ...
  return {
    ...
    sort_no: Number.isFinite(form.sort_no) ? form.sort_no : 0,
    status: form.status || "active",
    remark: form.remark?.trim() || undefined
