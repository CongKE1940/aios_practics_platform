...
func (repo *MySQLRepository) CountSchoolDeleteDependencies(ctx context.Context, tenantID int64, id int64) (SchoolDeleteDependencies, error) {
	...
	if err := repo.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM grades WHERE tenant_id = ? AND school_id = ? AND deleted_at IS NULL`, tenantID, id).Scan(&dependencies.GradeCount); err != nil {
		return SchoolDeleteDependencies{}, err
	}
	if err := repo.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM classes WHERE tenant_id = ? AND school_id = ? AND deleted_at IS NULL`, tenantID, id).Scan(&dependencies.ClassCount); err != nil {
		return SchoolDeleteDependencies{}, err
	}
	if err := repo.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM student_profiles WHERE tenant_id = ? AND school_id = ? AND enrollment_status <> 'left'`, tenantID, id).Scan(&dependencies.StudentCount); err != nil {
		return SchoolDeleteDependencies{}, err
	}
...
func (repo *MySQLRepository) ListGrades(ctx context.Context, tenantID int64, filter GradeListFilter) (PageResult[Grade], error) {
	query := `SELECT id, tenant_id, school_id, code, name, grade_level, school_year, status, created_at, updated_at FROM grades WHERE tenant_id = ? AND deleted_at IS NULL`
	args := []any{tenantID}
	if filter.SchoolID > 0 { query += " AND school_id = ?"; args = append(args, filter.SchoolID) }
	if filter.Status != "" { query += " AND status = ?"; args = append(args, filter.Status) }
	query += " ORDER BY id"
	rows, err := repo.db.QueryContext(ctx, query, args...)
	if err != nil { return PageResult[Grade]{}, err }
	defer rows.Close()
	items := make([]Grade, 0)
	for rows.Next() { grade, err := scanGrade(rows); if err != nil { return PageResult[Grade]{}, err }; items = append(items, grade) }
	if err := rows.Err(); err != nil { return PageResult[Grade]{}, err }
	return paginateItems(items, filter.Page, filter.PageSize), nil
}

func (repo *MySQLRepository) GetGrade(ctx context.Context, tenantID int64, id int64) (Grade, error) {
	const query = `SELECT id, tenant_id, school_id, code, name, grade_level, school_year, status, created_at, updated_at FROM grades WHERE id = ? AND tenant_id = ? AND deleted_at IS NULL LIMIT 1`
	grade, err := scanGradeScanner(repo.db.QueryRowContext(ctx, query, id, tenantID))
	if err != nil { return Grade{}, wrapNotFound(err) }
	return grade, nil
}
...
func (repo *MySQLRepository) CreateGrade(ctx context.Context, grade Grade) (Grade, error) {
	...
	if err != nil { return Grade{}, err }
	id, err := result.LastInsertId(); if err != nil { return Grade{}, err }
...
func (repo *MySQLRepository) UpdateGrade(ctx context.Context, grade Grade) (Grade, error) {
	...
	if err := repo.execAffectingOne(ctx, query, grade.SchoolID, grade.Code, grade.Name, grade.GradeLevel, nullString(grade.SchoolYear), grade.Status, grade.ID, grade.TenantID); err != nil { return Grade{}, err }
...
func (repo *MySQLRepository) ListClasses(ctx context.Context, tenantID int64, filter ClassListFilter) (PageResult[Class], error) {
	query := `SELECT id, tenant_id, school_id, grade_id, code, name, class_no, status, created_at, updated_at FROM classes WHERE tenant_id = ? AND deleted_at IS NULL`
	args := []any{tenantID}
	if filter.SchoolID > 0 { query += " AND school_id = ?"; args = append(args, filter.SchoolID) }
	if filter.GradeID > 0 { query += " AND grade_id = ?"; args = append(args, filter.GradeID) }
	if filter.Status != "" { query += " AND status = ?"; args = append(args, filter.Status) }
	query += " ORDER BY id"
	rows, err := repo.db.QueryContext(ctx, query, args...)
	if err != nil { return PageResult[Class]{}, err }
	defer rows.Close()
	items := make([]Class, 0)
	for rows.Next() { classItem, err := scanClass(rows); if err != nil { return PageResult[Class]{}, err }; items = append(items, classItem) }
	if err := rows.Err(); err != nil { return PageResult[Class]{}, err }
	return paginateItems(items, filter.Page, filter.PageSize), nil
}
...
func (repo *MySQLRepository) GetClass(ctx context.Context, tenantID int64, id int64) (Class, error) {
	...
	if err != nil { return Class{}, wrapNotFound(err) }
...
func (repo *MySQLRepository) CreateClass(ctx context.Context, classItem Class) (Class, error) {
	...
	if err != nil { return Class{}, err }
	id, err := result.LastInsertId(); if err != nil { return Class{}, err }
...
func (repo *MySQLRepository) UpdateClass(ctx context.Context, classItem Class) (Class, error) {
	...
	if err := repo.execAffectingOne(ctx, query, classItem.SchoolID, classItem.GradeID, classItem.Code, classItem.Name, nullInt(classItem.ClassNo), classItem.Status, classItem.ID, classItem.TenantID); err != nil { return Class{}, err }
...
func (repo *MySQLRepository) ListCourses(ctx context.Context, tenantID int64, filter CourseListFilter) (PageResult[Course], error) { return PageResult[Course]{Items: []Course{}, Page: normalizePage(filter.Page), PageSize: normalizePageSize(filter.PageSize), Total: 0}, nil }
func (repo *MySQLRepository) GetCourse(ctx context.Context, tenantID int64, id int64) (Course, error) { return Course{}, ErrNotFound }
func (repo *MySQLRepository) CreateCourse(ctx context.Context, course Course) (Course, error) { return Course{}, ErrInvalidInput }
func (repo *MySQLRepository) UpdateCourse(ctx context.Context, course Course) (Course, error) { return Course{}, ErrInvalidInput }
func (repo *MySQLRepository) DisableCourse(ctx context.Context, tenantID int64, id int64) error { return ErrNotFound }
...
func (repo *MySQLRepository) execAffectingOne(ctx context.Context, query string, args ...any) error {
	...
	rowsAffected, err := result.RowsAffected(); if err != nil { return err }
...
func scanSchoolScanner(scanner interface{ Scan(dest ...any) error }) (School, error) { var school School; err := scanner.Scan(&school.ID, &school.TenantID, &school.ObjectType, &school.Code, &school.Name, &school.EnglishName, &school.Address, &school.LogoURL, &school.Status, &school.CreatedAt, &school.UpdatedAt); return school, err }
func scanGrade(rows *sql.Rows) (Grade, error) { return scanGradeScanner(rows) }
func scanGradeScanner(scanner interface{ Scan(dest ...any) error }) (Grade, error) { var grade Grade; var schoolYear sql.NullString; err := scanner.Scan(&grade.ID, &grade.TenantID, &grade.SchoolID, &grade.Code, &grade.Name, &grade.GradeLevel, &schoolYear, &grade.Status, &grade.CreatedAt, &grade.UpdatedAt); if schoolYear.Valid { grade.SchoolYear = schoolYear.String }; return grade, err }
func scanClass(rows *sql.Rows) (Class, error) { return scanClassScanner(rows) }
func scanClassScanner(scanner interface{ Scan(dest ...any) error }) (Class, error) { var classItem Class; var classNo sql.NullInt64; err := scanner.Scan(&classItem.ID, &classItem.TenantID, &classItem.SchoolID, &classItem.GradeID, &classItem.Code, &classItem.Name, &classNo, &classItem.Status, &classItem.CreatedAt, &classItem.UpdatedAt); if classNo.Valid { value := int(classNo.Int64); classItem.ClassNo = &value }; return classItem, err }

func nullString(value string) any { if strings.TrimSpace(value) == "" { return nil }; return value }
func nullTime(value *time.Time) any { if value == nil { return nil }; return *value }
func nullInt(value *int) any { if value == nil { return nil }; return *value }
func wrapNotFound(err error) error { if err == nil { return nil }; if err == sql.ErrNoRows { return ErrNotFound }; return err }
